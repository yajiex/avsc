...
ClientChannel.prototype._emit = function (wreq, ctx, cb) {
  ...
  process.nextTick(function () {
    if (!msg.name) {
      ...
      self.emit('outgoingRequestPre', wreq, wres, ctx);
  ...
  function onTransition(wreq, wres, prev) {
    self.emit('outgoingRequestPost', wreq, wres, ctx);
    if (self.destroyed) {
    ...
    var id = self._registry.add(timeout, function (err, resPkt, adapter) {
      ...
      self.emit('incomingResponsePre', wreq, wres, ctx);
      prev(err);
    });
    id |= self._prefix;
    pkt.id = id;
  ...
  function onCompletion(err) {
    self.emit('incomingResponsePost', wreq, wres, ctx);
    self.pending--;
...
ServerChannel.prototype._receive = function (reqPkt, adapter, ctx, cb) {
  ...
  self.emit('incomingRequestPre', wreq, wreq, ctx);
  ...
  function onTransition(wreq, wres, prev) {
    self.emit('incomingRequestPost', wreq, wres, ctx);
      ...
      try {
        if (pending) {
          handler.call(ctx, wreq.request, function (err, res) {
            ...
            wres.error = err;
            wres.response = res;
            self.emit('outgoingResponsePre', wreq, wres, ctx);
            prev();
          });
        } else {
  ...
  function onCompletion(err) {
    self.pending--;
    self.emit('outgoingResponsePost', wreq, wres, ctx);
