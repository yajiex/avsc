...
Client.prototype.createChannel = function (transport, opts) {
  ...
  if (typeof transport == 'function') {
    ...
    if (objectMode) {
      ...
      callbackFactory = function (obj, cb) {
        var writable = transport.call(this, function (err, readable) {
          if (err) {
            cb(err);
            return;
          }
          onlyElement(readable.pipe(new FrameDecoder()), function (err, res) {
            if (err) {
              channel.destroy(err);
              return;
            }
    ...
    if (objectMode) {
      ...
      callbackFactory = function (fn) {
        transport.call(this, function (readable, writable) {
          ...
          onlyElement(readable.pipe(decoder), function (err, reqBuf) {
            if (err) {
              channel.destroy(err);
              return;
            }
            fn({payload: reqBuf}, function (err, resObj) {
              if (err) {
...
ClientChannel.prototype._emit = function (wreq, ctx, cb) {
  ...
  process.nextTick(function () {
    if (!msg.name) {
      ...
      self.emit('outgoingRequestPre', ctx, wreq, wres);
  ...
  function onTransition(wreq, wres, prev) {
    self.emit('outgoingRequestPost', ctx, wreq, wres);
    ...
    var id = self._registry.add(timeout, function (err, resBuf, adapter) {
      ...
      self.emit('incomingResponsePre', ctx, wreq, wres);
      prev(err);
    });
    id |= self._prefix;

    debug('sending message %s', id);
    self._send(id, reqBuf, ctx, !!msg && msg.oneWay);
  }

  function onCompletion(err) {
    self.emit('incomingResponsePost', ctx, wreq, wres);
    self.pending--;
...
StatelessClientChannel.prototype._send = function (id, reqBuf, ctx) {
  ...
  function emit(retry) {
    ...
    var req = Buffer.concat([HANDSHAKE_REQUEST_TYPE.toBuffer(hreq), reqBuf]);
    var reqObj = {id: id, context: ctx, payload: req};
    self._callbackFactory.call(self, reqObj, function (err, resObj) {
      if (err) {
        cb(err);
        return;
      }
      ...
      try {
        var parts = readHead(HANDSHAKE_RESPONSE_TYPE, resObj.payload);
        var hres = parts.head;
        if (hres.serverHash) {
...
StatefulClientChannel.prototype._send = function (id, reqBuf, ctx, oneWay) {
  if (oneWay) {
    var self = this;
  ...
  return this._writable.write({id: id, context: ctx, payload: [reqBuf]});
};
...
ServerChannel.prototype._receive = function (reqBuf, adapter, ctx, cb) {
  var self = this;
  var wreq;
  try {
  ...
  self.emit('incomingRequestPre', ctx, wreq, wres);
  ...
  function onTransition(wreq, wres, prev) {
    self.emit('incomingRequestPost', ctx, wreq, wres);
...
function StatelessServerChannel(server, callbackFactory, opts) {
  ...
  var self = this;
  callbackFactory.call(this, function (reqObj, cb) {
    if (self.destroyed) {
    ...
    var err;
    try {
      var parts = readHead(HANDSHAKE_REQUEST_TYPE, reqObj.payload);
      var hreq = parts.head;
    ...
    if (err) {
      ...
      self._receive(parts.tail, adapter, reqObj.context, done);
    }
    ...
    function done(resBuf) {
      ...
      var bufs = [HANDSHAKE_RESPONSE_TYPE.toBuffer(hres), resBuf];
      cb(null, {payload: Buffer.concat(bufs)});
    }
  });
}
...
function StatefulServerChannel(server, readable, writable, opts) {
  ...
  function onHandshake(obj) {
    ...
    if (err) {
      ...
      self._readable
        .removeListener('data', onHandshake)
        .on('data', onRequest);
      self._receive(parts.tail, self._adapter, obj.context, done);
    }
  ...
  function onRequest(obj) {
    ...
    var ctx = obj.context;
    self._receive(reqBuf, self._adapter, ctx, function (resBuf, oneWay) {
