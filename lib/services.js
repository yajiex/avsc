...
ClientChannel.prototype._emit = function (wreq, ctx, cb) {
  ...
  process.nextTick(function () {
    if (!msg.name) {
      ...
      self.emit('outgoingRequestPre', ctx);
  ...
  function onTransition(wreq, wres, prev) {
    self.emit('outgoingRequestPost', ctx, wreq.headers);
    ...
    var id = self._registry.add(timeout, function (err, resBuf, adapter) {
      ...
      self.emit('incomingResponsePre', ctx, wres ? wres.headers : undefined);
      prev(err);
    });
    id |= self._prefix;
  ...
  function onCompletion(err) {
    self.emit('incomingResponsePost', ctx, wres ? wres.headers : undefined);
    self.pending--;
...
ServerChannel.prototype._receive = function (reqBuf, adapter, cb) {
  ...
  var ctx = {channel: this};
  self.emit('incomingRequestPre', ctx);
  ...
  function onTransition(wreq, wres, prev) {
    self.emit('incomingRequestPost', ctx);
      ...
      try {
        if (pending) {
          handler.call(ctx, wreq.request, function (err, res) {
            ...
            wres.error = err;
            wres.response = res;
            self.emit('outgoingResponsePre', ctx);
            prev();
          });
        } else {
  ...
  function onCompletion(err) {
    self.pending--;
    self.emit('outgoingResponsePost', ctx);
