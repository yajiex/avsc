...
ClientChannel.prototype._emit = function (wreq, opts, cb) {
  ...
  function onTransition(wreq, wres, prev) {
    ...
    var timeout = utils.getOption(opts, 'timeout', self.timeout);
    ...
    self._send(id, reqBuf, opts, !!msg && msg.oneWay);
  }
...
StatelessClientChannel.prototype._send = function (id, reqBuf, opts) {
  ...
  function emit(retry) {
    ...
    var reqObj = {id: id, callOpts: opts, payload: req};
...
StatefulClientChannel.prototype._send = function (id, reqBuf, opts, oneWay) {
  if (oneWay) {
    var self = this;
  ...
  return this._writable.write({id: id, callOpts: opts, payload: [reqBuf]});
};
...
ServerChannel.prototype._receive = function (reqBuf, adapter, opts, cb) {
  var self = this;
  var wreq;
  try {
  ...
  var ctx = new CallContext(msg, this);
  self.emit('incomingCall', ctx, opts);
...
function StatelessServerChannel(server, callbackFactory, opts) {
  ...
  callbackFactory.call(this, function (reqObj, cb) {
    ...
    if (err) {
      ...
      self._receive(parts.tail, adapter, reqObj.callOpts, done);
    }
...
function StatefulServerChannel(server, readable, writable, opts) {
  ...
  function onHandshake(obj) {
    ...
    if (err) {
      ...
      self._readable
        .removeListener('data', onHandshake)
        .on('data', onRequest);
      self._receive(parts.tail, self._adapter, obj.callOpts, done);
    }
  ...
  function onRequest(obj) {
    ...
    var opts = obj.callOpts;
    self._receive(reqBuf, self._adapter, opts, function (resBuf, oneWay) {
