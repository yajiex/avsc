...
// TODO: Split "dynamic middleware" from `use` into a a separate `call` method
// (which can also be used to chain client modifiers which don't add a
// middleware).
// TODO: Introduce tags: decoded headers. They can be transferred automatically
// by being registered on a `tagTypes` client/server object. Wrapped request
// and response objects will have `tags` instead of `headers`.
...
function discoverProtocol(transport, opts, cb) {
  ...
  var ptclStr;
  svc.createClient({timeout: opts.timeout})
...
Client.prototype.createChannel = function (transport, opts) {
  ...
  if (typeof transport == 'function') {
    ...
    if (!newCodec) {
      ...
      // The transport should return a writable binary stream. As a
      // convenience, we provide a default standard-compliant codec. This
      // should support most use-cases (for example when speaking to the
      // official Java and Python implementations over HTTP, or when this
      // library is used for both the emitting and listening sides).
    ...
    if (codec) {
      ...
      codec.decoder.once('error', function (err) { channel.destroy(err); });
    }
  }
...
Client.prototype._emitMessage$ = function (wreq, ctx, cb) {
  ...
  ctx.channel = channel;
  channel._emit(wreq, ctx, function (err, wres) {
  ...
  if (typeof transport == 'function') {
    ...
    if (!newCodec) {
      ...
      callbackFactory = function (fn) {
        var codec = newCodec();
        transport.call(this, function (readable, writable) {
...
ClientChannel.prototype._emit = function (wreq, ctx, cb) {
  var msg = wreq._msg;
  ...
  process.nextTick(function () {
    if (!msg.name) {
      ...
      self.emit('outgoingRequestPre', wreq, ctx);
  ...
  function onTransition(wreq, wres, prev) {
    self.emit('outgoingRequestPost', wreq, ctx);
    if (self.destroyed) {
      prev(new Error('channel destroyed'));
      return;
    }
    try {
      var pkt = wreq.toPacket(0); // ID will be populated just below.
    } catch (err) {
      prev(err);
      return;
    }
    // Generate the response callback.
    var timeout = utils.getOption(ctx, 'timeout', self.timeout);
    var id = self._registry.add(timeout, function (err, resPkt, adapter) {
      if (!err && !msg.oneWay) {
        try {
          adapter._decodeResponse(resPkt, wres, msg);
        } catch (cause) {
          err = cause;
        }
      }
      self.emit('incomingResponsePre', wres, ctx);
      prev(err);
    });
    id |= self._prefix;
    pkt.id = id;
  ...
  function onCompletion(err) {
    self.emit('incomingResponsePost', wres, ctx);
    self.pending--;
...
StatelessClientChannel.prototype._send = function (baseReqPkt) {
  ...
  function emit(retry) {
    ...
    var reqPkt = baseReqPkt.withHandshake(hreq);
...
function StatefulClientChannel(client, readable, writable, opts) {
  ...
  function ping(retry) {
    ...
    self._writable.write(new Packet(self._prefix, null, null, hreq));
  }
...
ServerChannel.prototype._receive = function (reqPkt, adapter, ctx, cb) {
  var self = this;
  var id = reqPkt.id;
  var wreq;
  try {
    wreq = adapter._decodeRequest(reqPkt);
  } catch (cause) {
  ...
  if (!msg.name) {
    ...
    wres.response = null;
    cb(wres.toPacket(id), false);
    return;
  }

  ctx = utils.copyOwnProperties(ctx, {channel: this});
  self.emit('incomingRequestPre', wreq, ctx);
  ...
  function onTransition(wreq, wres, prev) {
    self.emit('incomingRequestPost', wreq, ctx);
      ...
      try {
        if (pending) {
          handler.call(ctx, wreq.request, function (err, res) {
            ...
            wres.error = err;
            wres.response = res;
            self.emit('outgoingResponsePre', ctx, wreq, wres);
            prev();
          });
        } else {
  ...
  function onCompletion(err) {
    self.pending--;
    self.emit('outgoingResponsePost', ctx, wreq, wres);
    ...
    if (!resPkt) {
      ...
      resPkt = self._systemErrorPacket(id, err, wres.headers);
...
ServerChannel.prototype._systemErrorPacket = function (id, err, headers) {
...
function StatelessServerChannel(server, callbackFactory, opts) {
  ...
  callbackFactory.call(this, function (reqPkt, ctx, cb) {
    ...
    if (err) {
      ...
      self._receive(reqPkt, adapter, ctx, done);
    }
...
function StatefulServerChannel(server, readable, writable, opts) {
  ...
  function onHandshake(reqPkt) {
    ...
    if (err) {
      ...
      self._readable
        .removeListener('data', onHandshake)
        .on('data', onRequest);
      self._receive(reqPkt, self._adapter, {}, done);
    }
  ...
  function onRequest(reqPkt) {
    ...
    self._receive(reqPkt, self._adapter, {}, function (resPkt, oneWay) {
...
WrappedRequest.prototype.toPacket = function (id) {
  var msg = this._msg;
  var body = Buffer.concat([
    STRING_TYPE.toBuffer(msg.name),
    msg.requestType.toBuffer(this.request)
  ]);
  return new Packet(id, this.headers, body);
};
...
WrappedResponse.prototype.toPacket = function (id) {
...
Adapter.prototype._decodeRequest = function (reqPkt) {
  ...
  return new WrappedRequest(msg, reqPkt.headers, req);
};

Adapter.prototype._decodeResponse = function (resPkt, wres, msg) {
  utils.copyOwnProperties(resPkt.headers, wres.headers, true);
...
function Packet(id, headers, body, handshake) {
  this.id = id;
  this.headers = headers || {};
  this.body = body || new Buffer([0, 0]); // Empty body.
  this.handshake = handshake;
}

Packet.forBuffer = function (id, buf, handshakeType) {
  var handshake, parts;
  if (handshakeType) {
    parts = readHead(handshakeType, buf);
    handshake = parts.head;
    buf = parts.tail;
  }
...
Packet.prototype.payload = function () {
  var bufs = [MAP_BYTES_TYPE.toBuffer(this.headers), this.body];
  if (this.handshake) {
    bufs.unshift(this.handshake.toBuffer());
  }
  return Buffer.concat(bufs);
};

Packet.prototype.withHandshake = function (handshake) {
  return new Packet(this.id, this.headers, this.body, handshake);
};
...
FrameDecoder.prototype._transform = function (buf, encoding, cb) {
  ...
  while (
    ...
    if (frameLength) {
      ...
      var pkt;
      try {
        pkt = Packet.forBuffer(0, Buffer.concat(bufs), this._handshakeType);
      } catch (err) {
        this.emit('error', err);
        return;
      }
...
function destroyTogether(channels, noWait) {
  channels.forEach(function (channel) { channel.once('eot', onEot); });
  function onEot() {
    channels.forEach(function (channel) { channel.destroy(noWait); });
  }
}
