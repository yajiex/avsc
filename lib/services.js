...
Service.prototype.createClient = function (opts) {
  ...
  process.nextTick(function () {
    ...
    if (opts && opts.server) {
      ...
      var inMemoryOpts = {objectMode: true, strictTypes: !!opts.strictTypes};
      var handler;
      opts.server.createChannel(function (fn) { handler = fn; }, inMemoryOpts);
      client.createChannel(
        function (cb) { return function (buf) { handler(buf, cb); }; },
        inMemoryOpts
      );
...
Client.prototype.createChannel = function (transport, opts) {
  ...
  if (typeof transport == 'function') {
    ...
    if (objectMode) {
      ...
      callbackFactory = function (cb) {
        var writable = transport(function (err, readable) {
          if (err) {
            cb(err);
            return;
          }
          onlyElement(readable.pipe(new FrameDecoder()), function (err, buf) {
            if (err) {
              channel.destroy(err);
              return;
            }
        ...
        if (writable) {
          ...
          return function (obj) { encoder.end(obj); };
        }
      };
    }
...
StatelessClientChannel.prototype._send = function (id, reqBuf) {
  ...
  function emit(retry) {
    ...
    var fn = self._callbackFactory.call(self, function (err, buf) {
      if (err) {
        cb(err);
        return;
      }
    ...
    if (!fn) {
      cb(new Error('transport error'));
      return;
    }
    fn(Buffer.concat([HANDSHAKE_REQUEST_TYPE.toBuffer(hreq), reqBuf]));
  }
};
